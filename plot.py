...
def plot_curve(psis, amplidues, n0, y_err, offset, i_color):
    ...
    curve.errorbar(psis[::marker_skip], amplidues[::marker_skip] + offset, fmt='x', markersize=3, yerr=y_err, capsize=1.5, color=color, zorder=1)
    thetas = np.linspace(-4, 4, 1000)
    curve.plot(thetas, 1/np.cosh(thetas)**2 + offset, c='k', ls='--', zorder=2)

def plot_resid(thetas, residuals, i_color):
    color = colors[i_color]
    resid.scatter(thetas[::marker_skip], residuals[::marker_skip], s=3, marker='x', color=color, zorder=1)

def plot_lag(residuals, i_color):
    color = colors[i_color]
    lag.scatter(residuals[:-1:lag_skip], residuals[1::lag_skip], marker='x', color=color, s=3, edgecolors='k', alpha=lag_alpha)
...
def plot_occ(all_residuals, bin_width):
    ...
    for i_bin, bin_start in enumerate(bins):
        count = all_residuals[np.where(np.logical_and(all_residuals > bin_start, all_residuals < bin_start + bin_width))].size / all_residuals.size
        if count > max_count:
            max_count = count
        occ.barh(bin_start, count, bin_width, align='edge', color=bincols[int(abs(len(bins)/2-i_bin))])
    return max_count
